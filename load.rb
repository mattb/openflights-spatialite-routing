...
db = SQLite3::Database.new "flights.sqlite"
db.enable_load_extension(1)
db.load_extension("/usr/local/lib/libspatialite.dylib")
db.execute("SELECT InitSpatialMetaData()")

puts "Loading airports..."
db.execute("create table airports (airport_id integer, name varchar(255), city varchar(255), country varchar(255), iata char(3), icao char(4), latitude float, longitude float, altitude integer, timezone float, dst char(1))")
...
db.close
`spatialite_network -d flights.sqlite -T routes -f source_airport_id -t destination_airport_id -c stops --unidirectional -g geom --output-table routes_stops_net_data --overwrite-output`
`spatialite_network -d flights.sqlite -T routes -f source_airport_id -t destination_airport_id --unidirectional -g geom --output-table routes_length_net_data --overwrite-output`
db = SQLite3::Database.new "flights.sqlite"
db.enable_load_extension(1)
db.load_extension("/usr/local/lib/libspatialite.dylib")
